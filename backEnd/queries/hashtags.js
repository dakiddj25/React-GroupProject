...
const getHashtagByHashtag = async (req, res, next) => {
    try {
        ...
        let hashtag = await db.any(`SELECT * FROM hashtags JOIN posts ON hashtags.post_id = posts.id LEFT JOIN users ON posts.user_id = USERs.id WHERE hashtag LIKE '%${hashtagId}%'`);
        res.status(200).json({
            status: "success",
